...
function SectionLabel({ index, title }: { index: string; title: string }) {
  return (
    <div className="border-hairline mb-8 flex items-baseline gap-3 border-b pb-3">
      <span className="text-accent text-xs tracking-[0.25em]">{index}</span>
      <h2 className="text-dim text-xs tracking-[0.25em] uppercase">{title}</h2>
    </div>
  );
}
...
function Keys({ children }: { children: string }) {
  return (
    <kbd className="inline-block rounded-[5px] border border-white/15 bg-white/[0.04] px-1.5 py-0.5 text-[12px] whitespace-nowrap">
      {children}
    </kbd>
  );
}
  ...
  return (
    <main className="mx-auto max-w-2xl px-6">
      ...
      <section className="py-20">
        <SectionLabel index="01" title="The premise" />
      ...
      <section className="py-20">
        <SectionLabel index="02" title="The specification" />
                ...
                <tbody>
                  {group.gestures.map((gesture) => (
                    ...
                    >
                      ...
                      <td className="text-dim py-2.5 text-[13px] leading-relaxed sm:text-sm">
                        {gesture.meaning}
                      </td>
                    </tr>
                  ))}
      ...
      <section className="py-20">
        <SectionLabel index="03" title="The philosophy" />
        <div className="text-dim space-y-5 leading-relaxed">
          <p>
          ...
          <p>
            ...
            feedback. Why windows you did not summon are left alone. And why the
            system teaches its own map: hold hyper, and every destination shows
            itself.
          </p>
          <p>
            ...
            , where every platform assumption was probed before it was trusted.
          </p>
        </div>
      </section>

      <section className="py-20">
        <SectionLabel index="04" title="Install" />
        <div className="text-dim space-y-5 leading-relaxed">
          <CopyCommand command="brew install --cask vaccone-software/tap/lodestar" />
          <p>
            Open it, grant Accessibility, and Lodestar wakes on its own. Hyper
            is right ⌘, which stops being a command key. That is the trade, and
            it is configurable.
          </p>
          <p>
      ...
      <footer className="border-hairline text-faint flex flex-col items-center gap-4 border-t py-16 text-center text-xs">
        ...
        <p>
          ...
          {" · "}FSL 1.1 with an MIT future grant{" · "}© 2026 Vaccone Software
          Company
        </p>
      </footer>
    </main>
