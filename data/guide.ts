...
export const guide: GuidePage[] = [
  ...
  },
  {
    ...
    finePrint: [
      ...
      "A copy that is only a color, a timestamp, a measurement or a sum keeps its text where every card does, and Lodestar leaves a note beneath it in its own serif: a color's name beside a swatch, how long ago a moment was with your clock and any zones you keep, a measurement in your units, or the answer. The note never pastes.",
      "Search finds a color by its name, so orange finds the orange you copied as a hex.",
    ...
    options: ["clipboard.enabled", "clipboard.save-to", "clipboard.exclude", "clipboard.exclude-apps", "clipboard.time-zones", "app.units", "clipboard.max-size-mb"],
  },
  {
    slug: "say",
    name: "What you say",
    ...
    ready: [
      {
        title: "Revise what a field already holds",
        keys: ["lode", "⇧."],
        rule: "The field's text, or its selection, is pulled into the draft, silent. Revise it from the keys, and return puts it back exactly where it was.",
        scene: "edit",
      },
      {
      ...
      },
      {
        ...
        table: [
          ...
          ["]s · [s", "To the next or the previous editor mark."],
          ["z= · zg", "Take the mark's fix, or keep the word as written."],
          ["u", "Undo."],
  ...
  },
  {
    ...
    first: {
      title: "Fix a mistake",
      keys: ["lode", "⇥"],
      rule: "Lodestar reads the field you are writing in and draws one thin line under a misspelled word or a slip of grammar. lode ⇥ puts a letter on every line, beside the fix. The letter applies it. A field with no lines has been read and is fine.",
      scene: "editor",
    },
    ready: [
      {
        title: "Keep what you wrote",
        keys: ["lode", "⇥", "⇧", "A"],
        lit: ["⇥", "⇧"],
        rule: "Shift and the letter says the words are right. A word the dictionary did not know becomes one of your words, shared with the draft. A grammar mark goes quiet in that sentence and teaches nothing, because the same change is right in one sentence and wrong in the next.",
      },
      {
        title: "Take a fix back",
        keys: ["lode", "⇥", "⌫"],
        lit: ["⇥", "⌫"],
        rule: "Delete in the lens undoes the last fix, and puts your cursor back where it was.",
      },
      {
        title: "Answer with the mouse",
        keys: [],
        lit: [],
        rule: "Rest the pointer on a line and a small card says what would change, with Accept and Keep as written. It never takes the focus, so the fix lands where your cursor was.",
        hidden: true,
      },
      {
        title: "Choose how much it reads",
        keys: [],
        lit: [],
        rule: "Settings has a page for the editor. Spelling needs no model at all. Minimal uses Apple's own. Standard is a 3.6 GB download that holds about 4 GB while you write, on a Mac with 16 GB. Full reads most precisely, on a Mac with 64 GB. A model too big for your Mac is shown, greyed, with what it needs.",
      },
    ],
    finePrint: [
      ...
      "In the draft, the same marks appear under what you dictate, and vim's spelling keys work: ]s to the next mark, z= to take its fix, zg to keep the word.",
      "The model loads when you start typing and lets its memory go two minutes after you stop, or at once when macOS says memory is short. It downloads in the background, checked file by file, never over a metered connection.",
      "Your text never leaves the Mac and is never kept. What Lodestar records is counts: marks shown, fixed and kept, by app.",
    ],
    why: "Silence is the statement that all is well: no badge, no score, no panel beside the text. A key that fixed everything was rejected, because names are exactly where a checker is confidently wrong, and a fix you did not see is not one you agreed to. So every mark is its own letter, and the lens stays until the last one is answered.",
    options: ["editor.enabled", "editor.model", "editor.language", "editor.skip-apps", "draft.words"],
  },
  {
    slug: "coach",
    name: "The coach",
